...
var (
	...
	// PaymentsProcessed counts the number of payments processed
...
// RecordPaymentProcessed records a payment processing metric
func RecordPaymentProcessed(status, method string) {
	PaymentsProcessed.WithLabelValues(status, method).Inc()
}

// RecordPaymentAmount records a payment amount
func RecordPaymentAmount(amount float64, currency string) {
